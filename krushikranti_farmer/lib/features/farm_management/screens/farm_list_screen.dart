...
import '../../dashboard/widgets/setup_progress_card.dart';
...
class _FarmListScreenState extends State<FarmListScreen> {
  ...
  Map<int?, Map<String, dynamic>> _fieldOfficerAssignments =
  ...
  Future<void> _loadFarms() async {
    ...
    try {
      final setupState = await SetupStateService.load();
      ...
      if (mounted) {
        setState(() {
          _setupState = setupState.copyWith(hasFarm: (data as List).isNotEmpty);
          _farms = data
      ...
      if (mounted) {
        setState(() {
          ...
          if (errorMsg.contains("Farmer profile not found") ||
            ...
            _setupState =
                _setupState.copyWith(hasProfile: false, hasFarm: false);
          }
        });
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _farms.isEmpty
              ? _buildIncompleteFarmState(l10n)
              : RefreshIndicator(
                  onRefresh: () async {
                    await _loadFarms();
                  },
  ...
  Widget _buildFarmCard(Farm farm, AppLocalizations l10n) {
    ...
    final bool isVerifiedFarm = farm.isVerified == true ||
        (fieldOfficerAssignment != null &&
            (fieldOfficerAssignment['status']?.toString().toUpperCase() ??
                    '') ==
                'COMPLETED');
        ...
        child: Column(
          ...
          children: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildInfoRow(
                      ...
                      "${farm.village}${farm.district != null ? ', ${farm.district}' : ''}",
                      AppColors.brandGreen),
                  const SizedBox(height: 12),
                  _buildInfoRow(
                      Icons.square_foot_rounded,
                      "${farm.totalAreaAcres} ${l10n.acres}",
                      AppColors.brandGreen),
                  if (farm.farmType != null) ...[
                    const SizedBox(height: 12),
                    _buildInfoRow(
                        Icons.eco_rounded,
                        _getLocalizedFarmType(farm.farmType!, l10n),
                        AppColors.brandGreen),
                  ],
                  if (farm.landOwnership.isNotEmpty) ...[
                    ...
                    _buildInfoRow(
                        Icons.home_rounded,
                        "${l10n.ownership}: ${_getLocalizedOwnership(farm.landOwnership, l10n)}",
                        AppColors.brandGreen),
                  ],
                ],
              ),
            ...
            if (farm.soilType != null ||
                farm.irrigationType != null ||
                farm.pincode.isNotEmpty) ...[
              ...
              if (farm.pincode.isNotEmpty)
                _buildInfoRow(Icons.pin_rounded,
                    "${l10n.pincode}: ${farm.pincode}", Colors.grey.shade600),
              if (farm.taluka != null && farm.taluka!.isNotEmpty) ...[
                const SizedBox(height: 12),
                _buildInfoRow(Icons.location_city_rounded,
                    "${l10n.taluka}: ${farm.taluka}", Colors.grey.shade600),
              ],
              if (farm.state != null && farm.state!.isNotEmpty) ...[
                const SizedBox(height: 12),
                _buildInfoRow(Icons.public_rounded,
                    "${l10n.state}: ${farm.state}", Colors.grey.shade600),
              ],
              if (farm.soilType != null) ...[
                const SizedBox(height: 12),
                _buildInfoRow(
                    Icons.terrain_rounded,
                    "${l10n.soilType}: ${_getLocalizedSoilType(farm.soilType!, l10n)}",
              ...
              if (farm.irrigationType != null) ...[
                ...
                _buildInfoRow(
                    ...
                    "${l10n.irrigationType}: ${_getLocalizedIrrigationType(farm.irrigationType!, l10n)}",
                    Colors.grey.shade600),
              ],
            ],
            ...
            if (farm.surveyNumber != null ||
              ...
              const SizedBox(height: 12),
              if (farm.surveyNumber != null &&
                  farm.surveyNumber!.isNotEmpty) ...[
                _buildInfoRow(
                    Icons.description_rounded,
                    "${l10n.surveyNo}: ${farm.surveyNumber}",
                    Colors.grey.shade600),
                const SizedBox(height: 12),
              ],
              if (farm.landRegistrationNumber != null &&
                  farm.landRegistrationNumber!.isNotEmpty) ...[
                _buildInfoRow(
                    Icons.assignment_rounded,
                    "${l10n.landRegNo}: ${farm.landRegistrationNumber}",
                    Colors.grey.shade600),
                const SizedBox(height: 12),
              ],
              ...
              if (farm.estimatedLandValue != null) ...[
                _buildInfoRow(
                    ...
                    "${l10n.estimatedValue}: ₹${farm.estimatedLandValue!.toStringAsFixed(2)}",
                    Colors.grey.shade600),
                const SizedBox(height: 12),
              ],
              if (farm.encumbranceStatus != null) ...[
                _buildInfoRow(
                    Icons.info_outline_rounded,
                    "${l10n.encumbrance}: ${_getLocalizedEncumbrance(farm.encumbranceStatus!, l10n)}",
                    Colors.grey.shade600),
              ],
              if (farm.encumbranceRemarks != null &&
                  farm.encumbranceRemarks!.isNotEmpty) ...[
                const SizedBox(height: 12),
                _buildInfoRow(
                    Icons.note_rounded,
                    "${l10n.remarks}: ${farm.encumbranceRemarks}",
                    Colors.grey.shade600),
              ],
            ],
  ...
  Widget _buildFieldOfficerAssignmentCard(
      Map<String, dynamic> assignment, AppLocalizations l10n) {
      ...
      child: Column(
        ...
        children: [
          ...
          if (fieldOfficerPhone.isNotEmpty ||
              fieldOfficerPincode.isNotEmpty ||
              assignedDateStr.isNotEmpty) ...[
  ...
  Widget _buildIncompleteFarmState(AppLocalizations l10n) {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        'Add your farm details to unlock weather insights and features.',
                        style: GoogleFonts.poppins(
                          fontSize: 13,
                          color: Colors.grey.shade600,
          ...
          // Unified Complete Setup Tracker
          SetupProgressCard(
